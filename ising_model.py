...
#    coupling_strengths = [1]
    ...
    for (idx, c) in enumerate(coupling_strengths):
        ...
        TEST_PWR_2_BOUND = False
        if TEST_PWR_2_BOUND:
            (barvinok_LB, pwr_2_LB, barvinok_UB) = upper_bound_Z_barvinok_k2(sg_model, k_2=k_2, TEST_PWR_2_BOUND=TEST_PWR_2_BOUND)
            barvinok_pwr2_LBs.append(pwr_2_LB)
        else:
            (barvinok_LB, barvinok_UB, delta_bar) = upper_bound_Z_barvinok_k2(sg_model, k_2=k_2, TEST_PWR_2_BOUND=TEST_PWR_2_BOUND)
            barvinok_pwr2_LBs.append(-1)
            if FIX_LOG_2:
                our_estimators.append(delta_bar*np.log(2.0))
                barvinok_LB *= np.log(2.0)
                barvinok_UB *= np.log(2.0)
            else:
...
#    ax.plot(coupling_strengths, barvinok_UBwmaxs, 'gx', label='our upper bound wmax')
    ...
    ax.plot(coupling_strengths, barvinok_UBs, 'b+', label='our upper bound')
    ax.plot(coupling_strengths, barvinok_LBs, 'b^', label='our lower bound')
    ax.plot(coupling_strengths, our_estimators, 'bx', label='our estimators (delta_bar)')

    if TEST_PWR_2_BOUND:
        ax.plot(coupling_strengths, barvinok_pwr2_LBs, 'g^', label='our pwr2 lower bound')
    ax.plot(coupling_strengths, gumbel_UBs, 'r+', label='gumbel upper bound')
    if CALC_GUMBEL_LB:
    ...
    if TEST_WISHFUL_BARVINOK:
        ax.plot(coupling_strengths, wishful_barvinok_LBs, 'y^', label='barvinok enumerate E_c[log(s1)] lower bound')
...
#    ax.plot(coupling_strengths, gumbel_barvinok_diff, 'bx', label='gumbel_UB - our_UB')
    ...
    fig.savefig('ising_model', bbox_extra_artists=(lgd,), bbox_inches='tight')    
...
#        test_find_w_min_spin_glass()
    ...
    compare_upper_bounds(N=4, f=1.0, num_gumbel_trials=10, k_2=100, gumbel_LB_copy_factor=10)
