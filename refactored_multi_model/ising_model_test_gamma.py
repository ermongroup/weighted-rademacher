...
def spin_glass_perturbed_MAP_state(sg_model, perturb_ones, perturb_zeros, log_base=np.e, gamma=1.0):
    '''
    ...
    - log_base: we're finding max_x {gamma*log w(x) + f(x)} where f(x) is some perturbation
    ...
    - gamma: we're finding max_x {gamma*log w(x) + f(x)}, CHECK CORRECTNESS
    Outputs:
...
#                                    perturb_ones[r,c] - perturb_zeros[r,c]
            ...
            lamda_i = (gamma*sg_model.lcl_fld_params[r,c]/np.log(log_base) + perturb_ones[r,c]) \
                    - (-gamma*sg_model.lcl_fld_params[r,c]/np.log(log_base) + perturb_zeros[r,c])
...
#            g.add_tedge(nodes[r*N + c], perturbed_0_potential, perturbed_1_potential)
    ...
    for r in range(N):
        for c in range(N):
            ...
            if c < N-1:
                g.add_edge(nodes[r*N + c], nodes[r*N + c+1], 2*gamma*sg_model.cpl_params_h[r,c]/np.log(log_base),
                           2*gamma*sg_model.cpl_params_h[r,c]/np.log(log_base))
                edge_count += 1
            #add a vertical edge
            if r < N-1:
                g.add_edge(nodes[r*N + c], nodes[(r+1)*N + c], 2*gamma*sg_model.cpl_params_v[r,c]/np.log(log_base),
                           2*gamma*sg_model.cpl_params_v[r,c]/np.log(log_base))
                edge_count += 1
...
def lower_bound_Z_extra_info(delta_bar, n, k, log_base, w_min=None, verbose=False, force_w_min_LB=False, w_max=None):
    '''
...
################### Calculate high probability lower bound ###################
    ...
    if w_min:
        ...
        lamda_min_hp = (scaled_delta_bar - np.sqrt(6*n/k) - log_w_min)/n
        if lamda_min_hp <= 1.0 or force_w_min_LB: 
            lower_bound_w_min = (scaled_delta_bar - np.sqrt(6*n/k) - log_w_min)**2/(2*n) + log_w_min
    lower_bound_no_w_min = scaled_delta_bar - np.sqrt(6*n/k) - n/2
    if force_w_min_LB or (w_min and lamda_min_hp <= 1.0):
        ...
        if verbose:
            print "high probability LOWER BOUND CALCULATION: used w_min, lambda =", lamda_min_hp
        high_prob_LB = lower_bound_w_min
    else:
        if verbose:
            print "high probability LOWER BOUND CALCULATION: did not use w_min, lambda =", lamda_min_hp
...
################### Calculate lower bound that holds in expectation for comparison with gumbel ###################
    ...
    if w_min:
        log_w_min = np.log(w_min)/np.log(log_base)
        lamda = (scaled_delta_bar - log_w_min)/n
        if lamda <= 1.0: 
            lower_bound_w_min = (scaled_delta_bar - log_w_min)**2/(2*n) + log_w_min
    lower_bound_no_w_min = scaled_delta_bar - n/2
    if w_min and lamda <= 1.0:
        #this should be true for lower bound using \Delta, but is it with this using \delta?
        assert(lower_bound_w_min >= lower_bound_no_w_min) 
        if verbose:
            print "expectation LOWER BOUND CALCULATION: used w_min: used w_min, lambda =", lamda
        expectation_LB = lower_bound_w_min
    else:
        if verbose:
            print "expectation LOWER BOUND CALCULATION: did not use w_min, lambda =", lamda
        expectation_LB = lower_bound_no_w_min

################### Calculate Stefano's idea ###################

    scaled_delta_bar = delta_bar*np.log(2)/np.log(log_base)

    if w_max:
        log_w_max = np.log(w_max)/np.log(log_base)        
        test_LB = (scaled_delta_bar - np.sqrt(6*n/k) - log_w_max)**2/(4*n) + log_w_max

    print "test_LB =", test_LB
    print "high_prob_LB =", high_prob_LB

    return (high_prob_LB, expectation_LB, lamda_min_hp, test_LB)


def upper_bound_Z_barvinok_k2(sg_model, k_2, log_base, compute_min_weight, gamma, force_w_min_LB):
    '''
    Upper bound the partition function of the specified spin glass model using
    ...
    for i in range(k_2):
        ...
        map_state = spin_glass_perturbed_MAP_state(sg_model, perturb_ones, perturb_zeros, log_base=2, gamma=gamma)
...
##End debugging
    ...
    if compute_min_weight:
        ...
        print '!'*8, "minimum weight =", w_min
    print '!'*8, "maximum weight =", w_max
...
###############testing done
    ...
    (high_prob_lower_bound, expectation_lower_bound, lamda_min_hp, test_LB) = lower_bound_Z_extra_info(delta_bar=delta_bar, \
        n=total_num_random_vals, k=k_2, log_base=log_base, w_min=w_min, verbose=True, force_w_min_LB=force_w_min_LB, w_max=w_max)

    scaled_delta_bar = delta_bar*np.log(2)/np.log(log_base)

    scaled_upper_guess = None

    return (high_prob_lower_bound, expectation_lower_bound, high_prob_upper_bound, expectation_upper_bound, scaled_delta_bar, scaled_upper_guess, log_w_max, lamda_min_hp, test_LB)
...
#            gumbel_LB = lower_bound_Z_gumbel(sg_model, gumbel_LB_copy_factor)
        ...
        (high_prob_barvinok_LB, expectation_barvinok_LB, high_prob_barvinok_UB, expectation_barvinok_UB, \
            delta_bar, upper_guess, log_w_max, lamda_1_min_hp, test_LB) = upper_bound_Z_barvinok_k2(sg_model, k_2=k_2, log_base=np.e, compute_min_weight=False, gamma=gamma, force_w_min_LB=False)
#        (high_prob_barvinok_LB_2_wmin_forced, expectation_barvinok_LB_2, high_prob_barvinok_UB_2, expectation_barvinok_UB_2, \
#            delta_bar_2, upper_guess_2, log_w_max_2, lamda_1_min_hp_2) = upper_bound_Z_barvinok_k2(sg_model, k_2=k_2, log_base=np.e, compute_min_weight=True, gamma=gamma, force_w_min_LB=True)
#        lamda_1_LB_force_wmins.append(high_prob_barvinok_LB_2_wmin_forced)
        lamda_1_LB_force_wmins.append(-1)
#            delta_bar, upper_guess, log_w_max) = upper_bound_Z_barvinok_k2(sg_model, k_2=k_2, log_base=np.e, compute_min_weight=False, gamma=gamma)
#        (high_prob_barvinok_LB_gamma_2, expectation_barvinok_LB, high_prob_barvinok_UB, expectation_barvinok_UB, \
#            delta_bar, upper_guess, log_w_max, lamda_2_min_hp) = upper_bound_Z_barvinok_k2(sg_model, k_2=k_2, log_base=np.e, compute_min_weight=True, gamma=1/lamda_1_min_hp, force_w_min_LB=True)
#        lamda_2_min_hps.append(lamda_2_min_hp)
#        gamma_2_lbs.append(high_prob_barvinok_LB_gamma_2)

        test_LBs.append(test_LB)
        print "gamma =", gamma
        print "high_prob_barvinok_LB =", high_prob_barvinok_LB

        lamda_min_hps.append(lamda_1_min_hp)

        our_estimators.append(delta_bar)

        barvinok_UBs.append(high_prob_barvinok_UB)
        barvinok_LBs.append(high_prob_barvinok_LB)
...
##########plot HIGH PROBABILITY estimates of log(Z)##########
    ...
    ax.plot(coupling_strengths, barvinok_UBs, 'b+', label='our UB', markersize=10)
    ax.plot(coupling_strengths, barvinok_LBs, 'b^', label='our LB', markersize=7)
    ax.plot(coupling_strengths, test_LBs, 'gx', label='test LB', markersize=7)
...
#    ax.plot(coupling_strengths, guess_UBs, 'm+', label='guess upper', markersize=10)
    ...
    ax.plot(coupling_strengths, high_prob_gumbel_UBs, 'm+', label='gumbel UB', markersize=10)
    if CALC_GUMBEL_LB:
        ax.plot(coupling_strengths, high_prob_gumbel_LBs, 'm^', label='gumbel LB', markersize=7)
    if CALC_EXACT_Z:
...
#    ax.plot(coupling_strengths, gumbel_barvinok_diff, 'bx', label='gumbel_UB - our_UB')
    ...
    plot_name = 'ising_model_gamma_highProb_%dx%dmodel,gumbel_trials=%d,our_k=%d' % (N, N, num_gumbel_trials, k_2)
...
##############plot lambda min hp#################
    ...
    ax.plot(coupling_strengths, lamda_min_hps, 'bx', label='lamda_1_min', markersize=10)
    ax.plot(coupling_strengths, lamda_2_min_hps, 'rx', label='lamda_2_min', markersize=10)

    
    plt.title('lambda_min for High Probability Bounds on Z(w)')
    plt.xlabel('c (Maximum Coupling Parameter)')
    ...
    plot_name = 'ising_model_lambda_min_highProb_%dx%dmodel,our_k=%d' % (N, N, k_2)
...
##############plot lower bounds#################
    ...
    ax.plot(coupling_strengths, barvinok_LBs, 'b^', label='LB using delta_1', markersize=7)
    ax.plot(coupling_strengths, gamma_2_lbs, 'r^', label='LB using delta_2', markersize=7)
    ax.plot(coupling_strengths, lamda_1_LB_force_wmins, 'm^', label='LB using delta_1, use w_min w/o checking lambda_min', markersize=7)

    ax.plot(coupling_strengths, our_estimators, 'bx', label='our estimator', markersize=10)

    if CALC_EXACT_Z:
        ax.plot(coupling_strengths, exact_log_Zs, 'g*', label='exact log(Z)', markersize=6)


    
    plt.title('High Probability lower Bounds on Z(w)')
    plt.xlabel('c (Maximum Coupling Parameter)')
    plt.ylabel('lambda_min')
    #plt.legend()
    ...
    plot_name = 'ising_model_LB_delta2_%dx%dmodel,our_k=%d' % (N, N, k_2)
...
#        test_find_w_min_spin_glass()
    ...
    compare_upper_bounds(N=5, f=1.0, num_gumbel_trials=10, k_2=10, gumbel_LB_copy_factor=10)
